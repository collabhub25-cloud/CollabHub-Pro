    ...
    const handleGoogleSignIn = useGoogleLogin({
        onSuccess: async (tokenResponse) => {
            ...
            try {
                ...
                if (response.ok) {
                    const userRole = data.user?.role;
                    if (userRole && userRole !== selectedRole && !response.url.includes('signup')) {
                        // Just an FYI for login if they selected the wrong role, but still let them in
                        toast.info(`You are signed in as a ${userRole}. Redirecting to your dashboard...`);
                    }
                    login(data.user);
                    router.push(`/dashboard/${userRole || selectedRole}`);
                } else {
